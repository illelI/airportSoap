...
class BookingEndpoint {
    ...
    public BookingResponse book(@RequestPayload BookingRequest request) {
        ...
        try {
            bookingService.book(request.getPassengerName(), request.getPassengerSurname(), request.getFlightNumber(), request.getFrom(), request.getTo() ,request.getDepartureDate(), request.getSeatClass());
